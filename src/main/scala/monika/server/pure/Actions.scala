...
object Actions {
  ...
  def restrictWebsites(profile: Profile): Vector[Effect] = {
    Vector(
  ...
  def restrictProjects(ext: External, profile: Profile): Vector[Effect] = {
  ...
  def restrictPrograms(profile: Profile): Vector[Effect] = {
